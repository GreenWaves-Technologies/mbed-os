...
/*
 ...
 - Change mbed_cpy_nvic to do nothing when in GAP architecture
 - Change mbed_start_main to clear MCAUSE when error occurs.
 - Change __wrap_main to weak functions when there is no OS.
 - Change software_init_hook to not use it when there is no OS.
 */
...
extern  char fc_main_stack[MBED_CONF_APP_MAIN_STACK_SIZE];
#define _main_stack fc_main_stack
#else
...
#endif
#else
    #define ISR_STACK_SIZE ((uint32_t)0)
#endif
