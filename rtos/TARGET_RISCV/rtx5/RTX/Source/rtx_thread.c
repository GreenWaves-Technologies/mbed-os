...
/*
 ...
 - Add svcRtxThreadNew context registers initilaization support for GAP architecture
 */
#include "rtx_lib.h"
...
static osThreadId_t svcRtxThreadNew (osThreadFunc_t func, void *argument, const osThreadAttr_t *attr) {
  ...
  if (thread != NULL) {
    ...
    thread->sp            = (uint32_t)stack_mem + stack_size - 30*4U;
    #else
    ...
    if ((osRtxConfig.flags & osRtxConfigStackWatermark) != 0U) {
      #if   (__RISCV_ARCH_GAP__   == 1U)
      for (n = (stack_size/4U) - (30U + 1U); n != 0U; n--) {
      #else
    ...
    #if   (__RISCV_ARCH_GAP__   == 1U)
    for (n = 0U; n != 27U; n++) {       // s0..s11, a0..a7, t0..t6, without hwlp_reg * 6
    #else
    for (n = 0U; n != 13U; n++) {
    #endif
