...
#if(__RISCV_ARCH_GAP__ == 1U)
__STATIC_INLINE uint32_t SysTick_Timer_Conf(char enable, char reset, char irq_enable, char event_mask, char cmp_clr, char one_shot, char prescaler_enable, char prescaler, char mode_64)
{
  return
      (enable << SysTick_CFG_REG_LOW_ENABLE_Pos)
